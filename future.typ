...
Additionally, we explored using equality saturation for a crucial part of type
checking and inference: unification. While equality saturation is highly
extensible, and as such was able to perform first-order syntactic unification as
well as equational unification, it struggled with runtime performance and
soundness. Namely, the runtime of algebraic simplification is too slow to
realistically be used in #rise's type system, and detecting unsatisfiability in
a set of unification goals raised many issues that we were not able to solve.
Because of these issues, we presented an alternative solution to perform
equational unification for #rise, namely SymPy. #rise's needs to unify terms of
natural numbers containing variables were precisely met by SymPy's ability to
solve systems of equations. However, while SymPy does detect unsatisfiability
(with the caveat of not being proven correct), it does not offer production of
an unsatisfiable core of the system in question, making it difficult to explain
to the user why a specific program did not typecheck.
...
into account, since both SymPy and equality saturation seem unsuitable for this.
Ideally, constraints could be synthesized by such a method and reported back to
the user (as in "This program does not typecheck unless the constraint
...
One method that addresses multiple shortcomings of equality saturation and SymPy
regarding equational unification over natural numbers is using an SMT solver.
...
to use equality saturation for synthesis, i.e., production of a unification
substitution, and an SMT solver for verification, explainability, and
constraints. However, this would still not perform arithmetic simplification,
which we need for a practical solution.
...
equality saturation. While we did face challenges specifically with equational
unification of natural numbers, we found that equality saturation is fully
capable of first-order syntactic unification. In the future, we intend to
explore whether it may support the type checking process in other ways, or help
with other parts of a language framework such as execution semantics.
Additionally, there are existing projects and ongoing research around extensions
to E-Graphs and symbolic computation that we want to explore for our purposes:
...
- `egglog` is a programming language that combines ideas from equality
  saturation and Datalog, a database query language. It shows large speedup
  running `egg`'s `math` test suite @zhangBetterTogetherUnifying2023 compared to
  `egg`, which could potentially be fast enough for our purposes.
